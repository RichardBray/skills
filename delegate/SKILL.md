...
This skill's routing assumes a strong orchestrator (opus-4.8+, sonnet-5+ or fable-5) doing decomposition and synthesis, with cheap models on leaf work - that split is what makes delegation a net win rather than just moving cost around.
...
- **gpt-5.x**: codex's sandbox cannot git-commit inside worktrees - the orchestrator commits for it. Bare `gpt-5.6` is not a valid model id (400s); use `gpt-5.6-sol` (frontier), `-terra` (balanced), or `-luna` (fast/cheap) on codex >= 0.144.x. On model-id errors or a stale codex install, read `references/codex-troubleshooting.md`.
- **fable-5**: ~2x opus cost and token burn; weak one-shot frontend/UI design and spec/PRD writing despite top intelligence. If fable-5 is the current orchestrator, delegating to it is pointless - keep that work in the main agent.
...
**Courier vs raw Bash - pick per task, not by default:** couriers are first-class subagents but also a relay through a weaker model (haiku) that can occasionally drop a formatting instruction (e.g. skip the `[model | status]` prefix). Raw Bash is synchronous and blocks your turn, but what you read is the literal CLI output - nothing summarized or possibly mangled in between.
...
**Live wezterm panes:** couriers stay the default delegation path.
