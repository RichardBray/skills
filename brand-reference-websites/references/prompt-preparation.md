...
Select a small, suitable reference shortlist using the catalog and the retrieval rules in SKILL.md. Include exact URLs/record paths and relevant sections. Separate observed patterns from candidates needing inspection; do not pretend motion was verified. When useful for selected visual references, read the available Firecrawl website design clone skill, retain branding/images/screenshot evidence and keep per-reference reports separate from the client's eventual DESIGN.md. Animation, transitions, scrolling and 3D require their own evidence. Avoid re-scraping adequate existing captures just to fill the template.
