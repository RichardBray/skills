...
3. Save branding and capture metadata under library/evidence. Keep source URL, capture date and viewport. Apply the [durable screenshot contract](library-model.md#discovery-and-seed-coverage-contract). Do not bypass media display restrictions. Never assume a URL was visually inspected.
4. Inspect the screenshot and chosen live sections. For motion, collect observations using the [motion contract](animation-patterns.md#motion-contract) and serialize them with the [library evidence schema](library-model.md#animation-fields-and-browsing); keep unobserved fields unknown. Capture both immediate and settled states; a single still cannot establish animation. Save an action log when durable media is unavailable and mark that limitation. Inspect a mobile viewport. Use the browser tooling available in the environment; fall back from incomplete scrapes to live interaction.
