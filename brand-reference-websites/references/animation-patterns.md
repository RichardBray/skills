...
For each effect chosen for a client build, specify:
...
- Renderer/asset plan: CSS/SVG, image layers, video, image sequence, shader or realtime 3D; keep source technology unknown unless verified. State licences, missing models and mobile resource constraints.
...
Prototype the defining motion with representative client assets before building all sections. Inspect its beginning, middle, end, reverse, interrupted and reduced-motion states. Use the same action sequence on source and implementation at comparable widths. Preserve the intended feel and user control; copy neither unmeasured timings nor source usability defects.
