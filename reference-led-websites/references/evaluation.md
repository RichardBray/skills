...
- A rich reference database is not proof of strong outputs.
- Validate the workflow through client-like builds when the user requests a trial or supplies an actual project.
- Do not silently build three extra websites during ordinary reference ingestion.
...
- Without revealing which is which, ask the user to judge:
  ...
  - motion quality.
- Apply the same [review criteria](review.md) to each build and verify that its source-to-element map accurately describes the output.

- Change the skill only in response to observed failures.
- Do not add a universal aesthetic rule because one client disliked an effect.
