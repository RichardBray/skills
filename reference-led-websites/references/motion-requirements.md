...
Before implementation, complete the canonical [motion contract](animation-patterns.md#motion-contract) in DESIGN.md. Inspect the reference behaviour directly: screenshots establish composition, not motion. Match assets to the interaction, including cutouts or models when needed; do not substitute rectangular photographs when the defining composition requires isolated objects.
...
Adapt the treatment to the site: a soft lift/scale for product details, a side drawer on desktop or bottom sheet on mobile for a basket, and a brief directional fade between steps are options, not a universal template. Keep routine quantity updates steady instead of replaying the whole entrance. Include closing as well as opening in the motion contract, and preserve the interaction semantics checked in [motion verification](review.md#motion-verification). Use subtle fades or immediate state changes for reduced motion. Do not delay essential actions for decorative sequencing.
